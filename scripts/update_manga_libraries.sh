...
MANGA_LIST_FILE="assets/manga_list.txt"
...
if $SCRIPT_DIR/Manga-Batch-Downloader/scripts/git_Manga-Batch-Downloader.sh; then
...
echo "[$(date)] Running command: python3 manga-batch-downloader.py --export-dir $DOWNLOADS --manga-list $MANGA_LIST_FILE --max-containers $MAX_CON" >> $LOG_FILE

# Run the batch download script
if python3 manga-batch-downloader.py --export-dir $DOWNLOADS --manga-list $MANGA_LIST_FILE --max-containers $MAX_CON; then
